...
public class MediaFileListFragment extends AbstractListFragment {
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    try {
      outState.putParcelableArrayList(PATH_CONTENTS,
          (ArrayList<FileLocation>) ((MediaFileListAdapter) getAdapter()).getFileItemList());
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    if (args != null) {
      ...
      customPath = args.getString(CUSTOM_PATH);
    }
  ...
  private void browseSources() {
    if (TextUtils.isEmpty(customPath)) {
      ...
      String[] properties = new String[] {
          ...
          ListType.FieldsFiles.SIZE, ListType.FieldsFiles.LASTMODIFIED,
          ListType.FieldsFiles.MIMETYPE
      };
      Files.GetDirectory action = new Files.GetDirectory(customPath, mediaType, sortMethod, properties);
    ...
    action.execute(hostManager.getConnection(), new ApiCallback<List<ListType.ItemFile>>() {
      ...
      public void onSuccess(List<ListType.ItemFile> result) {
        ...
        if (dir.hasParent) {
          ...
          FileLocation fl = new FileLocation("返回", parentDirectory, true);
  ...
  public static class FileLocation implements Parcelable {
    ...
    public void setRootDir(boolean root) {
      this.isRoot = root;
    }
    ...
    public FileLocation(String title, String path, boolean isDir, String details,
        String sizeDuration, String artUrl) {
      this.title = title;
      this.file = path;
    ...
    public static FileLocation newInstanceFromItemFile(Context context,
        ListType.ItemFile itemFile) {
      ...
      switch (itemFile.type) {
        ...
        case ListType.ItemBase.TYPE_EPISODE:
          title = itemFile.title;
          details = String.format(context.getString(R.string.season_episode), itemFile.season,
              itemFile.episode);
          sizeDuration = (itemFile.size > 0) && (itemFile.runtime > 0) ?
              UIUtils.formatFileSize(itemFile.size) + " | " + UIUtils.formatTime(itemFile.runtime) :
              (itemFile.size > 0) ? UIUtils.formatFileSize(itemFile.size) :
    ...
    public static final Parcelable.Creator<FileLocation> CREATOR =
        new Parcelable.Creator<FileLocation>() {
